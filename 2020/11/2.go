	...
	for i := -1; i <= 1; i++ {
		for j := -1; j <= 1; j++ {
			...
			for true {
				dirX += i
				dirY += j
				if x + dirX > maxX || x + dirX < 0 || y + dirY > maxY || y + dirY < 0 {
					break
				} else if seatMap[x+dirX][y+dirY] == occupied {
					result++
					break
				} else if seatMap[x+dirX][y+dirY] == empty {
					break
				}
			}
		}
	...
	for true {
		...
		for i := 0; i < x; i++ {
			for j := 0; j < y; j++ {
				...
				} else if seatMap[i][j] == occupied && neighbourCount[i][j] >= 5 {
					seatMap[i][j] = empty
				}
			}
		}
